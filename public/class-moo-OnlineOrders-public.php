    ...
    public function moo_cart_getTotal()
    {
        if(isset($_SESSION['items']) && !empty($_SESSION['items'])){
            ...
            $response = array(
                ...
                'total_of_taxes'	=> round($total_of_taxes, 2,PHP_ROUND_HALF_UP),
...
/**
    ...
    public function moo_modifier_add()
    {
        ...
        $modifiers = sanitize_text_field($_POST['modifiers']);
        $flag = false;
    ...
    private function moo_CreateOrder($ordertype,$taxable)
    {
        ...
        if($total['status']=='success'){
            ...
            if(isset($order->href)){
                ...
                foreach($_SESSION['items'] as $item)
                {
                    // If the item is empty skip to the next iteration of the boocle
    ...
    private function moo_PayOrder($card_number,$cvv,$expMonth,$expYear,$orderId,$amount,$taxAmount,$zip)
    {
        ...
        else
        {
            $response = array(
                'status'	=> 'Error',
                'message'	=> 'We have an internal error, please contact us via email'
            );
            wp_send_json($response);
        }
