...
class checkoutPage
{
    ...
    public function render($atts, $content)
    {
        ...
        if($this->pluginSettings["order_later"] == "on") {
            $inserted_nb_days = intval($this->pluginSettings["order_later_days"]);
            $inserted_nb_mins = intval($this->pluginSettings["order_later_minutes"]);

            $inserted_nb_days_d = intval($this->pluginSettings["order_later_days_delivery"]);
            $inserted_nb_mins_d = intval($this->pluginSettings["order_later_minutes_delivery"]);

            if($this->pluginSettings["order_later_days"] === "")
            {
                $nb_days = 4;
            }
            else
            ...
            if($this->pluginSettings["order_later_days_delivery"] === "")
            {
                $nb_days_d = 4;
            }
            else
        ...
        if($this->pluginSettings['hours'] != 'all' && $oppening_status->status == 'close') {
            if(isset($this->pluginSettings["closing_msg"]) && $this->pluginSettings["closing_msg"] !== '') {
                ...
                if($oppening_status->store_time == '')
                    $oppening_msg = '<div class="moo-alert moo-alert-danger" role="alert" id="moo_checkout_msg">Online Ordering Currently Closed'.(($this->pluginSettings['accept_orders_w_closed'] == 'on' )?"<br/><p style='color: green'>Order in Advance Available</p>":"").'</div>';
                else
                    $oppening_msg = '<div class="moo-alert moo-alert-danger" role="alert" id="moo_checkout_msg"><strong>Today\'s Online Ordering hours</strong> <br/> '.$oppening_status->store_time.'<br/>Online Ordering Currently Closed'.(($this->pluginSettings['accept_orders_w_closed'] == 'on' )?"<br/><p style='color: green'>Order in Advance Available</p>":"").'</div>';
            }
        }
                    ...
                    <div class="moo-col-md-6">
                        <form action="post" onsubmit="moo_login(event)">
                            <div class="form-group">
                                ...
                                <input type="text" id="inputEmail" class="moo-form-control" autocomplete="email">
                            </div>
                        ...
                        <div class="moo-shopping-cart MooCartInCheckout">
                            ...
                            <div class="moo-totals" style="padding-right: 10px;">
                                ...
                                <div class="moo-totals-item" id="MooDeliveryfeesInTotalsSection">
                                    <label class="moo-checkoutText-deliveryFees"><?php echo $this->pluginSettings["delivery_fees_name"];?></label>
                                ...
                                <div class="moo-totals-item" id="MooServiceChargesInTotalsSection"  style="<?php if(! $total['serviceCharges']>0) echo 'display:none;';?>">
                                    <label id="MooServiceChargesName"><?php echo $this->pluginSettings['service_fees_name']; ?></label>
